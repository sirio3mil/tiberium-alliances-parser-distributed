...
class Grabber
{
    ...
    public function __construct(CCApi $api)
    {
        $this->api = $api;
    }
    ...
    public function parse()
    {
        ...
        while ($cur < $last) {
            ...
            while (!$p['p']) {
                sleep(5);
                $p = (array)$this->api->getPlayers($cur, $next);
                    file_put_contents("c:\\attemp" . $this->api->getServer()."_$attemps", '');
                if (!$attemps) {
                    file_put_contents("c:\\grab" . $this->api->getServer(), '');
                    die;
                }
                $attemps--;
            }
    ...
    public function writeData()
    {
        ...
        foreach ($this->users as $user) {
            ...
            $players[$user['i']] = array(
                'bd' => $user['bd'], //Base destoryed total?
                'bde' => $user['bde'], //PvE destoed?
                'd' => $user['d'], // PvP Destroyed
        ...
        $dataFile = $path . $this->api->getServer() . ".json";
